...
static void HamiltonBEPRecursivo (size_t i, Datos& datos, const Grafo& grafo) {
    //Itero posibilidades
    Vertice va = grafo.PrimerVerticeAdyacente(datos.solActual[i]);
    const Vertice NULO;
    while (va!=NULO) {
        if (datos.diccionario.count(va.getEtiqueta())==0) { //Factible
            datos.solActual[i+1] = va;
            datos.costoActual+= grafo.Peso(datos.solActual[i], va);
        ...
        datos.costoActual-= grafo.Peso(datos.solActual[i], va); 
        va = grafo.SiguienteVerticeAdyacente(datos.solActual[i], va); 
    }
}
...
namespace Algoritmos
{
    std::vector<Vertice> HamiltonBEP (const Grafo& grafo) {
        Datos datos(grafo);
        HamiltonBEPRecursivo(0, datos, grafo);
